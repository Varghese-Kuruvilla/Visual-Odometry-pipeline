...
class KittiEvalOdom():
    ...
    def compute_ATE(self, gt, pred):
        ...
        for i in pred:
            ...
            # print("pred: ", pred_xyz)
            # input("debug")
            gt_xy = np.asarray([gt_xyz[0], gt_xyz[2]])
            pred_xy = np.asarray([pred_xyz[0], pred_xyz[2]])
            errors.append(np.linalg.norm(gt_xy - pred_xy))
            # errors.append(np.linalg.norm(gt_xyz - pred_xyz))
    ...
    def compute_RPE(self, gt, pred):
        ...
        for i in list(pred.keys())[:-1]:
            ...
            # if self.rotation_error(rel_err)>1:
            #     print(i)
            #     continue
            local_dist = np.linalg.norm(gt_rel[:3, 3])
            local_trans_error = self.translation_error(rel_err)
            trans_errors2.append(np.linalg.norm(gt_rel[:3, 3] - pred_rel[:3, 3]))
            tot_trans_dist += local_dist
            trans_errors.append(local_trans_error / local_dist )
        ...
        # print(sorted(np.asarray(rot_errors)))
        rpe_rot = np.mean(np.asarray(rot_errors))
        # print(rpe_rot)
        # print(tot_trans_dist, np.mean(trans_errors), np.sum(trans_errors) / tot_trans_dist)
        return rpe_trans, np.sum(rot_errors) * 180 / np.pi, tot_trans_dist
    ...
    def eval(self, result_dir,flag, alignment=None, seqs=None):
        ...
        seq_rpe_trans = []
        seq_rpe_rot = []
        file_name = 'resultall.txt'

        with open("../config/vo_params.yaml") as f:
            vo_params = yaml.load(f, Loader=yaml.FullLoader)
        vo_poses_filename = vo_params['poses_file_path'] + '.txt'
        poses_result = self.load_poses_from_txt(vo_poses_filename)

        gt_filename = vo_params ['gt_txt_file_path']
        modified_gt_filename = gt_filename.split('.txt')[0] + '_modified.txt'
        poses_gt = self.load_poses_from_txt(modified_gt_filename)
        self.result_file_name = result_dir+'resultall.txt'
        ...
        # Compute ATE
        ate_error = self.compute_ATE(poses_gt, poses_result)

        # Compute RPE
        rep_error, rot_error, total_distance = self.compute_RPE(poses_gt, poses_result)

        return (ate_error/total_distance, rep_error, rot_error / total_distance , total_distance)


if __name__ == '__main__':
    keo = KittiEvalOdom()
    print(keo.eval('resdir',1, alignment='6dof'))
