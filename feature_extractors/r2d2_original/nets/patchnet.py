...
import logging
logger = logging.getLogger('module_patchnet')
...
class BaseNet (nn.Module):
    ...
    def forward(self, imgs, **kw):
        ...
        # logger.info("res before merging:"+str(res))
        # logger.info("res[descriptors].size():" + str(res[0]['descriptors'].size()))
        # logger.info("res[repeatability].size():"+ str(res[0]['repeatability'].size()))
        # logger.info("res[reliability].size()" + str(res[0]['reliability'].size()))
        # breakpoint
        # merge all dictionaries into one
        res = {k:[r[k] for r in res if k in r] for k in {k for r in res for k in r}}
        # logger.info("res after merging:"+str(res))
