...
class Camera: 
    def __init__(self, width, height, fx, fy, cx, cy, D=np.asarray([0,0,0,0,0]), fps = 15): # D = [k1, k2, p1, p2, k3]
        self.width = width
        self.height = height
        self.fx = fx
        self.fy = fy
...
class PinholeCamera(Camera):
    def __init__(self, width, height, fx, fy, cx, cy, D=np.asarray([0,0,0,0,0]), fps = 15):
    ...
    def cam2pix(self, pts_3D):
        u = self.fx * pts_3D[...,0]/pts_3D[...,2] + self.cx
        v = self.fy * pts_3D[...,1]/pts_3D[...,2] + self.cy  
